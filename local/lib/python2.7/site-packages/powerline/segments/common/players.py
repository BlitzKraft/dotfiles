...
class PlayerSegment(Segment):
	def __call__(self, format='{state_symbol} {artist}', state_symbols=STATE_SYMBOLS, **kwargs):
		stats = {
			'state': 'fallback',
			'album': None,
			'artist': None,
...
class MpdPlayerSegment(PlayerSegment):
	def get_player_status(self, pl, host='localhost', port=6600):
		try:
			...
			now_playing = sp.call(['mpc', 'current','-f', '%file%/%time%','-h', str(host),'-p', str(port)])
			if not now_playing:
				return
